...
<?php
	...
	if($disable_stick_menu != 'on') {
		if(isset($ruya_options['stick_main_menu_v5']) && $ruya_options['stick_main_menu_v5']) {
	...
	<div class="mo-header-menu">
		<div class="container-fluid">
		 <div id="mo-header-icon" class="mo-header-icon visible-xs visible-sm"><span></span></div>
		   ...
		   <?php if ( isset($ruya_options['menu_other_v5']) AND $ruya_options['menu_other_v5'] !=''){ 
				if (is_array($ruya_options['menu_other_v5'])) {
					if ( in_array('lang', $ruya_options['menu_other_v5'], true)) { ?> 
						  <?php lang_link(); ?>
					<?php	}
				}
			} ?>
		   <div class="menu_other_v5 hidden-sm hidden-xs">
			    <?php if ( isset($ruya_options['menu_other_v5']) AND $ruya_options['menu_other_v5'] !=''){ 
					if (is_array($ruya_options['menu_other_v5'])) {
						if ( in_array('sidepanel', $ruya_options['menu_other_v5'], true)) { ?> 
                ...
                <?php if ( isset($ruya_options['menu_other_v5']) AND $ruya_options['menu_other_v5'] !=''){ 
					if (is_array($ruya_options['menu_other_v5'])) {
						if ( in_array('search', $ruya_options['menu_other_v5'], true)) { ?> 
							 <div class="mo-search-header">
								<a class="search-trigger"><i class="fa fa-search"></i><span class="search_txt"><?php echo esc_html__('search', 'ruya'); ?></span></a>
							</div>
						<?php	}
					}
				} ?>
			   <?php 
				global $woocommerce;
				if ( isset($ruya_options['menu_other_v5']) AND $ruya_options['menu_other_v5'] !=''){ 
					if (is_array($ruya_options['menu_other_v5'])) {
						if ( in_array('cart', $ruya_options['menu_other_v5'], true)) { ?> 
                ...
                <?php if ( isset($ruya_options['menu_other_v5']) AND $ruya_options['menu_other_v5'] !=''){ 
					if (is_array($ruya_options['menu_other_v5'])) {
						if ( in_array('social', $ruya_options['menu_other_v5'], true)) { ?> 
							 <ul class="social-header-v5 social_list"><?php ruya_top_social(); ?></ul> 
						<?php	}
					}
				} ?>
                <?php if ( isset($ruya_options['menu_other_v5']) AND $ruya_options['menu_other_v5'] !=''){ 
					if (is_array($ruya_options['menu_other_v5'])) {
						if ( in_array('button', $ruya_options['menu_other_v5'], true)) { ?> 
							<a class="btn-nav button large light hr_light bg_secondary bg_hr_secondary gradient-anim" href="<?php echo esc_url($ruya_options['menu_other_but_link_v5']);?>"><span><?php echo esc_html($ruya_options['menu_other_but_v5']);?></span></a>
						<?php	}
					}
				} ?>
