...
/* ----------------------------------------------*
   excerpt
/* ----------------------------------------------*/
if(!function_exists('ruya_excerpt_length')) {
	/*
	 * Custom excerpt length
	 */
	function ruya_excerpt_length( $length ) {
		return 30;
	}
}

if(!function_exists('ruya_posts_link_attributes_1')) {
	/*
	 * post link attribute
	 */
	function ruya_posts_link_attributes_1() {
		return 'class="older"';
	}
}

if(!function_exists('ruya_posts_link_attributes_2')) {
	/*
	 * post link attribute
	 */
	function ruya_posts_link_attributes_2() {
		return 'class="newer"';
	}
}
/*-----------------------------------------------*
  POST DIRECTION
/*-----------------------------------------------*/
if ( ! function_exists( 'ruya_post_directions' ) ) :
    function ruya_post_directions( ) {
	    ...
	    $prev_post_link = $next_post_link = $next_post_title = $prev_post_title = $url_prev = $url_next = false; ?>
            <div class="post-paginations">
			   ...
			   if(!empty($prev_post) && is_object($prev_post)) {
				    ...
				    $url_prev = wp_get_attachment_url( get_post_thumbnail_id($prev_post->ID),'ruya-small'); 
			   }
               $prev_post_link != '' ? '' : 'empty';
		       if(!empty($prev_post_link)) :
					echo ' <div class="post-pagi prev"><a href="'. esc_url($prev_post_link) .'">';
		                echo '<div class="pagi_nav"><span class="arrow"></span> <span class="nav_dir">'. esc_html__( 'PREVIOUS POST', 'ruya' ) .'</span></div>';
				        echo '<div class="pagi_details">';
		                   if(!empty($url_prev)){ echo '<img src="'.esc_url($url_prev).'" alt="'.esc_attr__('image','ruya').'"/>'; }
               ...
               echo '<a href="'.$homeLink .'" class="pagi-icon-grid"><div class="icon"></div></a>'; ?>
                ...
                <?php if(!empty($next_post) && is_object($next_post)) {
				    ...
				    $url_next  = wp_get_attachment_url( get_post_thumbnail_id($next_post->ID),'ruya-small'); 
				}  
	            $next_post_link != '' ? '' : 'empty';
				if(!empty($next_post_link)) :
					echo '<div class="post-pagi next"><a href="'. esc_url($next_post_link) .'">';
		                echo '<div class="pagi_nav"><span class="arrow"></span> <span class="nav_dir">'. esc_html__( 'NEXT POST', 'ruya' ) .'</span></div>';
		                echo '<div class="pagi_details">';
		                   if(!empty($url_next)){ echo '<img src="'.esc_url($url_next).'" alt="'.esc_attr__('image','ruya').'"/>'; }
            ...
            <div class="related-post">
                 <div class="grid-mid-post">
					<div class="format-post">
						<figure>
						   <a href="<?php echo get_the_permalink(); ?>"><?php the_post_thumbnail("ruya-small"); ?></a>
...
/*-----------------------------------------------*
  Post gallery
/*-----------------------------------------------*/
if (!function_exists('ruya_grab_ids_from_gallery')) {
    function ruya_grab_ids_from_gallery() {
        global $post;
        $gallery = ruya_get_shortcode_from_content('gallery');
        $object = new stdClass();
        $object->columns = '3';
        $object->link = 'post';
        $object->ids = array();
        if ($gallery) {
            $object = ruya_extra_shortcode('gallery', $gallery, $object);
        }
        return $object;
    }
}
/*-----------------------------------------------*
  Custom comment list
/*-----------------------------------------------*/
function ruya_custom_comment($comment, $args, $depth) {
...
<div class="comment-body">
    ...
    <?php if ( $comment->comment_approved == '0' ) : ?>
        <em class="comment-awaiting-moderation"><?php esc_html_e( 'Your comment is awaiting moderation.', 'ruya' ); ?></em>
    <?php endif; ?>
    <?php comment_text(); ?>
