		...
		switch ($format) {
			...
			case 'gallery':
				...
				if(!empty($attachment_ids)) {
					...
					foreach($attachment_ids as $attachment_id ) {
						$image_attributes = wp_get_attachment_image_src($attachment_id, 'ruya-small');
			...
			case 'video':
				...
				if (has_post_thumbnail()) {
					$media_output .= '
					<figure>
					  '.get_the_post_thumbnail(get_the_ID(), "ruya-small").'
			...
			case 'audio':
				...
				if($audio_source_from == 'soundcloud') {
					...
					$media_output .= '<figure><a href="'.get_the_permalink().'">'.get_the_post_thumbnail(get_the_ID(), "ruya-small").'</a> <a class="cat-name" href="'. esc_url(get_term_link($terms[0]->slug, 'category')) .'">'.  esc_html($terms[0]->name) .'</a>';
					if($audio_url) {
