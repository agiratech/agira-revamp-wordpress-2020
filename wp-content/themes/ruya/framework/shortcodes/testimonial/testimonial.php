...
if(function_exists('ruya_shortcode')) { ruya_shortcode('testimonial', 'motivoweb_testimonial_carousel_func'); }
// single_testimonial
function motivoweb_single_testimonial_func($atts, $content = null) {
	...
	<div class="item">
        <?php $attachment_image = wp_get_attachment_image_src($image, 'full', false); 
		   if($attachment_image[0]) { echo '<div class="testimonial-avatar"><figure><img src="'.esc_url($attachment_image[0]).'" alt="'.esc_html($author).'"/></figure></div>';
		} ?>
		<div class="testimonial-details">
		    <?php if($rating) echo '<span class="star-rating '.($star_rating).'"></span>'; ?>
			<div class="content"><?php if($details) echo '<p>'.($details).'</p>'; ?></div><!-- content  --> 
...
if(function_exists('ruya_shortcode')) { ruya_shortcode('single_testimonial', 'motivoweb_single_testimonial_func'); }