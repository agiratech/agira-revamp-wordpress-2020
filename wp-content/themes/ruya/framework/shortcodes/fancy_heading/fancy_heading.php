<?php
function ruya_fancy_heading_func($atts, $content = null) {
	...
	// rotate
	wp_enqueue_script('textillate', RUYA_URI_PATH . '/assets/js/textillate.js');
    $lists = array();
	    ...
	    <div class="bs-textillate-selector tlt"  data-textillate-handle='<?php echo esc_attr( $_tlt_settings ); ?>'>
		     <ul class="texts">
				<?php foreach ($lists as $key => $value){ ?>
				    <?php if( count($word) && !empty($value['word']) ) { ?>	<li><?php echo esc_html($value['word']); ?></li><?php } ?>
				<?php } ?>
			</ul>
		</div>
		<?php } ?>	
...
if(function_exists('ruya_shortcode')) { ruya_shortcode('fancy_heading', 'ruya_fancy_heading_func'); }