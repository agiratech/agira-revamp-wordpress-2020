...
class WPCal_Bookings_Query{
	...
	private static function _get_bookings($args){
		...
		$query = "SELECT $after_select $select_cols_imploded FROM `$table_wpcal_bookings` ";
		...
		if(!empty($args['filters']['text'])){
			if(is_numeric($args['filters']['text'])){
				$numeric_value = $args['filters']['text'];
				$query_where.= $wpdb->prepare( " AND `id` = %s", $numeric_value );
			}
			else{
				$text_value = $args['filters']['text'];
