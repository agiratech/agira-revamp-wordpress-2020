...
class WPCal_Mail{
	...
	public static function send_admin_reschedule_booking_info(WPCal_Booking $booking_obj){
		...
		$reschedule_cancel_by = '';
		if( $reschedule_cancel_user_id ){
			if( $reschedule_cancel_user_id == $booking_obj->get_invitee_wp_user_id() ){
				$reschedule_cancel_by = $booking_obj->get_invitee_name();
			}
			elseif( $reschedule_cancel_user_id == $booking_obj->get_invitee_wp_user_id() ){
				$reschedule_cancel_by = $admin_details['display_name'] ? $admin_details['display_name'] : $admin_details['name'];
			}
			
			if( empty($reschedule_cancel_by) ){
				$_user = get_user_by( 'id', $reschedule_cancel_user_id );
				$_name = $_user->first_name . ' ' . $_user->last_name;
				$reschedule_cancel_by = trim($_name);
			}
		}
		...
		foreach($question_answers as $key => $question_answer){
			...
			<tr>
				<td style="padding: 10px 0;'. $first_q_a_style .'">
					...
					<div style="color: #7c7d9c;margin-top: 5px;white-space: pre-wrap;">'. $question_answer['answer'] .'</div>
				</td>
			</tr>';
