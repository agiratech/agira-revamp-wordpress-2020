...
class WPCal_TP_Google_Calendar extends WPCal_Abstract_TP_Calendar{
    ...
    private function event_time_to_unix($event_time){
        ...
        $time_obj = WPCal_DateTime_Helper::DateTime_DB_to_DateTime_obj($t, new DateTimeZone('UTC'));
    ...
    private function _prepare_booking_event_object_for_api(WPCal_Booking $booking_obj, $add_to_cal_details){
        ...
        if( !empty($admin_details['display_name']) && !empty($invitee_name) ){
            $summary = sprintf( /* translators: 1: person1 name 2: person2 name */ __('%1$s and %2$s', 'wpcal'), $admin_details['display_name'], $invitee_name);
        }
        else{
        ...
        $location_descr = wpcal_get_booking_location_content($booking_obj, $for='calendar_event', $whos_view='neutral', $this->get_provider(), $location_content_options);
        ...
        $description = wpcal_get_booking_descr_for_calendar($service_obj, $booking_obj, $location_descr);
