...
function wpcal_get_current_user_for_booking_in_user_client(){
	...
	if( is_user_logged_in() ){
		...
		$_user_data['is_pre_fill_user_details'] = wpcal_is_pre_fill_user_details($user);
	}
	return $_user_data;
}

function wpcal_is_pre_fill_user_details($current_user){
	if( $current_user->has_cap('activate_plugins') ){
		return false;
	}
	return true;
}
...
function wpcal_mark_onboarding_checklist_completed_for_current_admin($task){
	...
	if( !in_array($task, ['add_calendar_account', 'add_meeting_app']) ){
		//invalid task
		return;
	}
...
function wpcal_after_add_booking_add_background_tasks(WPCal_Booking $booking_obj, $booking_action){
	...
	$expiry_ts = WPCal_DateTime_Helper::DateTime_Obj_to_unix($to_time_obj) + (60 * 60);//(60 * 60) 1 hr after booking "to" time
...
function wpcal_reschedule_booking($old_booking_id, $new_booking_data){
	...
	$cancel_reason = sanitize_text_field($reschedule_reason);
...
function wpcal_may_update_or_delete_booking_to_tp_calendar(WPCal_Booking $booking_obj, $do){
	...
	if( $cal_details->calendar_id != $calendar_id ){
		...
		$query = "SELECT `calendar_account_id` FROM `$table_calendars` WHERE `id` = %s";
		...
		if( empty($calendar_account_id) ){
			...
			$query2 = "SELECT `id` FROM `$table_calendar_accounts` WHERE `id` = %s";
			$query2 = $wpdb->prepare( $query2, $calendar_account_id );
...
function wpcal_get_services($options=[]){
	...
	$query = "SELECT `service`.`id`, `service`.`name`, `service`.`status`, `service`.`duration`, `service`.`relationship_type`, `service`.`post_id`, `service`.`color` FROM `$table_services` as `service` JOIN `$table_service_admins` as `service_admin` ON `service`.`id` = `service_admin`.`service_id` WHERE `service`.`status`IN ($statuses_escaped_imploded)";
...
function wpcal_google_calendar_receive_token_and_add_account(){
	...
	$tp_calendar_obj = new $tp_calendar_class(0);
	$calendar_account_id = $tp_calendar_obj->add_account_after_auth();
	...
	if( $calendar_account_id ){
		$calendar_page_link .= $calendar_account_id ? '/connected/'.$calendar_account_id : '';
	}
	wp_redirect($calendar_page_link);
	exit;
}

function wpcal_disconnect_calendar_by_id($calendar_account_id, $provider, $force=false){
	$list = [ 'google_calendar'];
	if(!in_array($provider, $list) ){
		throw new WPCal_Exception('invalid_tp_calendar_provider');
	}
	...
	$result = $tp_calendar_obj->revoke_access_and_delete_its_data($force);
	return $result;
}
...
function wpcal_get_conflict_calendar_ids_by_admin($admin_user_id){
	...
	$query = "SELECT `calendars`.`id` FROM `$table_calendars` as `calendars` JOIN `$table_calendar_accounts` as `calendar_accounts` ON `calendar_accounts`.`id` = `calendars`.`calendar_account_id` WHERE `calendar_accounts`.`status` = '1' AND `calendars`.`status` = '1' AND `calendars`.`is_conflict_calendar` = '1' AND `calendar_accounts`.`admin_user_id` = %s";
	$query = $wpdb->prepare( $query, $admin_user_id );
...
function wpcal_update_add_bookings_to_calendar_id_for_current_admin($add_bookings_to_calendar_id){
	...
	$query = "SELECT `id` FROM `$table_calendar_accounts` WHERE `status` = '1' AND `admin_user_id` = %s";
	$query = $wpdb->prepare( $query, $admin_user_id );
...
function wpcal_update_conflict_calendar_ids_for_current_admin($conflict_calendar_ids, $conflict_calendar_ids_length){
	...
	$query2 = "UPDATE `$table_calendars` SET `is_conflict_calendar` = '0' WHERE `is_conflict_calendar` = '1' AND `calendar_account_id` IN($calendar_account_ids_imploded)";//$calendar_account_ids no prepare sql required
...
function wpcal_tp_account_receive_token_and_add_account($provider){
	...
	$tp_class = wpcal_include_and_get_tp_class($provider);
	$tp_obj = new $tp_class(0);
	$tp_account_id = $tp_obj->add_account_after_auth();
...
function wpcal_get_active_tp_account_by_admin_and_provider($admin_user_id , $provider){
	global $wpdb;
	$table_tp_accounts = $wpdb->prefix . 'wpcal_tp_accounts';
	$query = "SELECT * FROM `$table_tp_accounts` WHERE `admin_user_id` = %s AND `provider` = %s AND `status` = '1'";
	$query = $wpdb->prepare( $query, $admin_user_id, $provider );
	$result = $wpdb->get_row($query);
	if( $result === false ){
		throw new WPCal_Exception('db_error', '', $wpdb->last_error);
...
function wpcal_get_booking_descr_for_calendar(WPCal_Service $service_obj, WPCal_Booking $booking_obj, $location_descr, $options=[]){
