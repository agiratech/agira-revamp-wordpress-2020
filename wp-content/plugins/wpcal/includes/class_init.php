...
var __wpcal_dist_url = "<?php echo WPCAL_PLUGIN_DIST_URL; ?>";
	...
	public static function tp_compatibility_on_init(){
		add_filter('option_aj_defer', 'WPCal_Init::tp_async_javascript_add_defer');//plugin async-javascript
		add_filter('pre_update_option_aj_defer', 'WPCal_Init::tp_async_javascript_remove_defer_in_db');//plugin async-javascript
	}

	private static function _tp_async_javascript_defer_url(){
		$defer_js_path_url = trailingslashit(WPCAL_PLUGIN_DIST_URL) . 'js';
		$site_url = trailingslashit(site_url());
		$defer_relative_url = wpcal_may_remove_prefix_str($defer_js_path_url, $site_url);
		return $defer_relative_url;
	}

	public static function tp_async_javascript_add_defer($value){
		
		$defer_relative_url = self::_tp_async_javascript_defer_url();
		$value_array = empty($value) ? [] : explode(',', $value);
		if( !in_array($defer_relative_url, $value_array) ){
			$value_array[] = $defer_relative_url;
			$value_array[] = 'i18n.js';
			$value_array[] = 'i18n.min.js';
		}
		$final_value = implode(',', $value_array);
		return $final_value;
	}

	public static function tp_async_javascript_remove_defer_in_db($value){
		//basically lets store the wpcal defer in DB, so we can update the defer setting later

		$defer_relative_url = self::_tp_async_javascript_defer_url();
		$value_array = empty($value) ? [] : explode(',', $value);
		$remove_values = [$defer_relative_url, 'i18n.js', 'i18n.min.js' ];
