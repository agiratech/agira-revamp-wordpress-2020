<?php
class ruya_Woo_Filter_Attribute extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Shows a custom attribute in a widget which lets you narrow down the list of products when viewing product categories.', 'ruya' );
		$this->widget_id          = 'woocommerce_layered_nav';
		$this->widget_name        = esc_html__( 'WooCommerce Filter Attribute', 'ruya' );
	...
	public function init_settings() {
		...
		$this->settings = array(
			'title' => array(
				'type'  => 'text',
				'std'   => esc_html__( 'Filter by', 'ruya' ),
				'label' => esc_html__( 'Title', 'ruya' )
			),
			'attribute' => array(
				'type'    => 'select',
				'std'     => '',
				'label'   => esc_html__( 'Attribute', 'ruya' ),
				'options' => $attribute_array
			),
			'display_type' => array(
				'type'    => 'select',
				'std'     => 'list',
				'label'   => esc_html__( 'Display type', 'ruya' ),
				'options' => array(
					'list'     => esc_html__( 'List', 'ruya' ),
					'dropdown' => esc_html__( 'Dropdown', 'ruya' ),
			...
			'query_type' => array(
				'type'    => 'select',
				'std'     => 'and',
				'label'   => esc_html__( 'Query type', 'ruya' ),
				'options' => array(
					'and' => esc_html__( 'AND', 'ruya' ),
					'or'  => esc_html__( 'OR', 'ruya' )
				)
			),
		);
	}
		...
		if ( 0 < count( $terms ) ) {
			...
			if ( 'dropdown' == $display_type ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					echo '<option value="">' . sprintf( esc_html__( 'Any %s', 'ruya' ), wc_attribute_label( $taxonomy ) ) . '</option>';
...
/**
 * Class ruya_Woo_Filter_Attribute
 */
function register_woo_filter_attribute_widget() {
    register_widget('ruya_Woo_Filter_Attribute');
}
