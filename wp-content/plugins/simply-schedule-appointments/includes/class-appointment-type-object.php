...
class SSA_Appointment_Type_Object {
	...
	public function get_max_per_day_schedule( Period $period, $args = array() ) {
		...
		if ( $period_tz->durationLessThan( Period::createFromDuration( $period_tz->getStartDate(), $one_day_interval ) )) {
			// we need to add a day on each end to make sure we have enough to calculate a full day of availability
			$period_tz = $period_tz->moveEndDate( new DateInterval( 'P2D' ) )->sub( $one_day_interval );
		}
	...
	public function get_appointment_schedule( Period $period, $args = array() ) {
		...
		ssa()->availability_cache->insert_schedule( $schedule, array(
